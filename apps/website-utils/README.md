...
A small suite of client-side browser tools, each a self-contained static HTML page (no build step). These are ports of the three tools from the v0.dev "Website Utils" app (deployed on Vercel at `https://v0-website-utils.vercel.app`), rebuilt so everything runs statically on GitHub Pages with behavior equivalent to the originals.

## Tools

| Page | Tool | How it works |
|---|---|---|
| `index.html` | Landing page | Links to the three tools below. |
...
- `tool1.html`, `tool2.html`, `tool3.html` — each tool in a single file (UI, CSS, and logic together).
