/* iso.js: isometric projection and solid primitives for the tissue engineering
   simulation. Grid space: x grows toward the lower-right, y toward the lower-left,
   z up. One grid unit ≈ 1 metre of lab floor.

   The look serves a dark industrial floor lit from above-left, with a hard dark
   outline on every solid — machinery reading as stamped, chunky objects rather
   than a soft render. */
(function (global) {
  'use strict';
  ...
  /* Accepts "#rgb", "#rrggbb" and "rgb(r,g,b)". The rgb() form is a guard:
     feeding one to parseInt(.., 16) yields NaN, which shades to solid black,
     and on a dark floor that failure is completely invisible. */
  ...
  /* Rounded rectangle path (2D HUD panels). Uses native roundRect when
     available, falls back to arcTo for older engines. */
  ...
  /* Cladding on the two camera-facing walls: ribbed steel with one lit band of
     glazing, which is what a bioreactor hall actually looks like from outside. */
  ...
  /* A box turned to face (hx, hy) along the floor. `len` runs along the
     ...
     line up with a belt: the pallet, machine arms, bioreactor plumbing. */
  ...
  /* A truncated cone: radius r0 at z, r1 at z + h. This is the bioreactor
     dome, pump volute, and tank taper. Also used for construct shapes.
