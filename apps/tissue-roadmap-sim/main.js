...
(function (global) {
  ...
  document.addEventListener('keydown',function(e){if(e.target.matches('input,button'))return;var k=e.key.toLowerCase();if(k===' '){e.preventDefault();Sim.togglePause();}if(k==='s')UI.advance();if(k==='r')UI.run();if(k==='f')setFollow(!follow);if(k==='l'){var b=document.getElementById('labels');b.checked=!b.checked;Render.setLabels(b.checked);}});
  ...
  function frame(now){requestAnimationFrame(frame);var dt=Math.max(0,Math.min(.05,(now-last)/1000));last=now;Sim.update(dt);var running=!Sim.state.paused&&Sim.state.running&&!Sim.state.finished;if(running)clock+=dt;var fly=UI.flyTarget(), target=fly?Iso.project(fly.x,fly.y,0):carrierTarget(), k=1-Math.pow(.05,dt);if(fly){cam.x+=(target.x-cam.x)*.18;cam.y+=(target.y-cam.y)*.18;cam.scale+=(1.15-cam.scale)*.18;if(Math.abs(target.x-cam.x)<.8&&Math.abs(target.y-cam.y)<.8)UI.clearFlyTo();}else if(follow){cam.x+=(target.x-cam.x)*k;cam.y+=(target.y-cam.y)*k;}syncOffsets();Render.draw(canvas,cam,clock,UI.pipelineStation(),hover,UI.inspectingStation(),Sim.getDwellProgress?Sim.getDwellProgress():0);UI.paint(false);}
